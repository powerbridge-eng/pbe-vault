...
# --- 2. SYSTEM DOCTOR (STABILITY & DATA INTEGRITY) ---
def init_system():
    conn = get_db()
    if not conn: return
    cur = conn.cursor()
...
def admin_dashboard():
    ...
    return render_template_string(BASE_HTML.replace("{% block content %}{% endblock %}", f"""
        ...
        <div class="layer-box">
            <div class="layer-title">👥 PERSONNEL REGISTRY (SOUL AUDIT READY)</div>
            <table style="width:100%; border-collapse:collapse; font-size:11px;">
                <thead><tr style="background:#f1f1f1;"><th style="padding:10px; text-align:left;">PBE-ID</th><th style="text-align:left;">NAME</th><th style="text-align:left;">ACTIONS (6)</th></tr></thead>
                <tbody>
                    {{% for w in workers %}}
        ...
        <div class="fab-zone">
            ...
            <a href="/admin/invite" class="fab fab-invite" title="Invite Personnel">+</a>
        </div>
    """), guilds=PBE_GUILDS, stats=stats, workers=workers, alerts=expiry_alerts)

# --- 6. ROBOT MAPPING & REGISTRY FORM ---
...
def register():
    if request.method == 'POST':
        ...
        # CLOUDINARY SMART NAMING (IDENTITY MAPPING)
...
def invite():
    if request.method == 'POST':
        ...
        requests.post("https://sms.arkesel.com/api/v2/sms/send", 
                     json={"sender": "PBE_OTP", "message": f"PBE: Use OTP {otp} to register: {request.url_root}register", "recipients": [phone]}, 
