...
def init_db():
    ...
    # Bridge: Ensure Memory Column exists
...
def log_soul_action(action, details):
    ...
    cur.execute("INSERT INTO pbe_audit_2026 (action, actor, details, ip_address, device_info) VALUES (%s, %s, %s, %s, %s)",
                (action, actor, details, ip, device))
    conn.commit(); cur.close(); conn.close()
...
def admin_dashboard():
    ...
    return render_template_string(BASE_HTML.replace("{% block content %}{% endblock %}", """
        <div class="search-container">
            ...
            <div style="background:#fff; padding:15px; border-radius:10px; border:1px solid #dee2e6; font-weight:bold;">SMS: <span style="color:green;">OFFLINE</span></div>
        </div>
        ...
        <div class="section-card">
            ...
            <div class="matrix-grid">
                ...
                <div class="matrix-item" style="text-align:left;">{{guild}} <b style="color:red; float:right;">{{count}}</b></div>
                {% endfor %}
            </div>
        </div>
        <div class="section-card">
                    ...
                    <tbody>
                        ...
                        <tr class="worker-row">
                            ...
                            <td>
                                ...
                                <a href="#" class="btn-cmd bg-blue">UNSUSPEND</a>
    ...
    """), reg_stats=reg_stats, guild_stats=guild_stats, workers=workers, alerts=expiry_alerts, role=role)

# (Remainder of login/invite/audit/alerts logic as provided in your original prayers...)
