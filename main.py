...
app = Flask(__name__)
app.secret_key = "PBE_SUPREME_FINAL_2026_MASTER"
...
# --- 2. THE SYSTEM DOCTOR (STABILITY CHECK) ---
def init_fresh_system():
    conn = get_db()
    if not conn: return
    cur = conn.cursor()
    # Ensuring all necessary tables and columns exist
...
<head>
    ...
    <style>
        ...
        .search-bar { width: 100%; padding: 15px; border-radius: 10px; border: 1px solid #ddd; font-size: 16px; box-sizing: border-box; margin-bottom: 20px; }
...
def admin_dashboard():
    ...
    return render_template_string(BASE_HTML.replace("{% block content %}{% endblock %}", f"""
        ...
        <div class="layer-box">
            <div id="registryList">
                {{% for w in workers %}}
                <div class="worker-row" style="padding:10px; border-bottom:1px solid #eee; font-size:12px;">
                    <b>{{{{ w[5] }}}}</b> | {{{{ w[1] }}}} {{{{ w[2] }}}}
                </div>
                {{% endfor %}}
            </div>
...
def invite():
    if request.method == 'POST':
        ...
        # CEO FIX: Clear any old stuck registration for this number so OTP works perfectly
        ...
        requests.post("https://sms.arkesel.com/api/v2/sms/send", 
                     json={"sender": "PBE_OTP", "message": f"PBE: Use OTP {otp} to register at: {request.url_root}register", "recipients": [phone]}, 
...
def register():
    if request.method == 'POST':
        ...
        if not user_row: return "INVALID OTP."
        
        photo = cloudinary.uploader.upload(request.files['photo'])
        now = datetime.datetime.now()
        uid = f"PBE{now.strftime('%y%m')}{request.form.get('firstname')[:3].upper()}{''.join(random.choices(string.digits, k=4))}"
        ...
        cur.execute("""UPDATE pbe_master_registry SET surname=%s, firstname=%s, pbe_uid=%s, pbe_license=%s, 
                    rank=%s, department=%s, email=%s, ghana_card=%s, photo_url=%s, 
                    region=%s, issuance_date=%s, expiry_date=%s, status='ACTIVE' WHERE otp_code=%s""",
                   (request.form.get('surname').upper(), request.form.get('firstname').upper(), uid, lic,
                    request.form.get('rank'), request.form.get('department'), request.form.get('email'), 
                    request.form.get('ghana_card'), photo['secure_url'], request.form.get('region'), 
    ...
    return render_template_string(BASE_HTML.replace("{% block content %}{% endblock %}", """
        <div class="layer-box" style="max-width:500px; margin:auto;">
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <select name="region" style="width:100%; padding:12px; margin-bottom:10px;">{% for r in regions %}<option value="{{r}}">{{r}}</option>{% endfor %}</select>
                <select name="department" style="width:100%; padding:12px; margin-bottom:10px;">{% for g in guilds %}<option value="{{g}}">{{g}}</option>{% endfor %}</select>
...
# --- AUDIT, ALERTS, AND LOGIN INTACT ---
@app.route("/admin/audit")
def view_audit():
    if not session.get('role'): return redirect(url_for('admin_login'))
    conn = get_db(); cur = conn.cursor(); cur.execute("SELECT * FROM pbe_soul_audit ORDER BY id DESC LIMIT 50")
    logs = cur.fetchall(); cur.close(); conn.close()
    return render_template_string(BASE_HTML.replace("{% block content %}{% endblock %}", """<div class="layer-box"><h3>📜 SOUL AUDIT</h3>{% for l in logs %}<div>[{{ l[1] }}] {{ l[3] }}: {{ l[4] }}</div>{% endfor %}<a href="/admin-dashboard" class="btn-6 bg-navy">BACK</a></div>"""), logs=logs)

@app.route("/admin/alerts")
def view_alerts():
    if not session.get('role'): return redirect(url_for('admin_login'))
    conn = get_db(); cur = conn.cursor(); cur.execute("SELECT * FROM pbe_master_registry WHERE expiry_date <= CURRENT_DATE + INTERVAL '30 days'")
    alerts = cur.fetchall(); cur.close(); conn.close()
    return render_template_string(BASE_HTML.replace("{% block content %}{% endblock %}", """<div class="layer-box"><h3>🔔 RENEWAL ALERTS</h3>{% for a in alerts %}<div>{{ a[1] }} {{ a[2] }} - Expires: {{ a[17] }}</div>{% endfor %}<a href="/admin-dashboard" class="btn-6 bg-navy">BACK</a></div>"""), alerts=alerts)
